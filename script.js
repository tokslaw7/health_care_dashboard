    ...
    async function fetchPatients() {
      ...
      try {
        ...
        selectedPatient = patients.find(patient => patient.name === "Jessica Taylor") || patients[0] || null;
        status.remove();
    ...
    function selectPatient(patient) {
      ...
      setImageSource("profilePhoto", patient.profile_picture , patient.name || "Selected patient");
      ...
      setTextContent("systolicTrend", bp.systolic?.levels ? `▲ ${bp.systolic.levels}` : "--");
      setTextContent("diastolicValue", bp.diastolic?.value ?? "--");
      setTextContent("diastolicTrend", bp.diastolic?.levels ? `▼ ${bp.diastolic.levels}` : "--");

      setTextContent("respiratoryRate", latest.respiratory_rate?.value ? `${latest.respiratory_rate.value} bpm` : "--");
      setTextContent("respiratoryStatus", latest.respiratory_rate?.levels || "--");
      setTextContent("temperature", latest.temperature?.value ? `${latest.temperature.value}°F` : "--");
      setTextContent("temperatureStatus", latest.temperature?.levels || "--");
      setTextContent("heartRate", latest.heart_rate?.value ? `${latest.heart_rate.value} bpm` : "--");
      setTextContent("heartStatus", latest.heart_rate?.levels ? `▼ ${latest.heart_rate.levels}` : "--");
    ...
    function renderChart(history) {
      const labels = history.map(item => `${monthShort[item.month] || item.month}, ${item.year}`);
      const systolic = history.map(item => item.blood_pressure?.systolic?.value || 0);
      const diastolic = history.map(item => item.blood_pressure?.diastolic?.value || 0);
    ...
    function renderDiagnosticList(items) {
      ...
      `).join("") : `<tr><td colspan="3">No diagnostic records available.</td></tr>`;
    }
    ...
    function renderLabResults(items) {
      ...
      `).join("") : `<div class="lab-item">No lab results available.</div>`;
    }
